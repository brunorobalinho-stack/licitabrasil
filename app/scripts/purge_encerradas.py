...
def _build_where(dias: int) -> str:
    """Monta clausula WHERE para licitacoes purgaveis.

    Patterns sao constantes hardcoded — sem risco de SQL injection.
    O parametro 'dias' vem de argparse (int validado).
    """
    ...
    return (
        ...
        f" OR l.data_publicacao < NOW() - INTERVAL '{dias} days')"
    )


async def get_stats(session, dias: int) -> dict:
    where = _build_where(dias)
    ...
    result = await session.execute(text(
        ...
        f" AND l.data_publicacao >= NOW() - INTERVAL '{dias} days'"
    ))
...
async def do_export_csv(session, dias: int) -> Path:
    ...
    result = await session.execute(text(
        f"SELECT {cols} FROM licitacoes l WHERE {where}"
    ))
