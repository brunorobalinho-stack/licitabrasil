...
describe('GET /api/licitacoes', () => {
  ...
  it('uses tsvector search when q is provided', async () => {
    mockPrisma.$queryRawUnsafe
      ...
      .mockResolvedValueOnce([{ ...sampleLicitacao, rank: 0.5, headline: 'Aquisição de <mark>equipamentos</mark>' }]); // data query
    ...
    // findMany should NOT have been called because tsvector succeeded
    expect(mockPrisma.licitacao.findMany).not.toHaveBeenCalled();
  });
  ...
  it('includes highlights map when q is provided', async () => {
    mockPrisma.$queryRawUnsafe
      ...
      .mockResolvedValueOnce([{ ...sampleLicitacao, rank: 0.5, headline: 'Aquisição de <mark>equipamentos</mark>' }]);

    const res = await request(app).get('/api/licitacoes?q=equipamentos');

    expect(res.status).toBe(200);
    expect(res.body.highlights).toBeDefined();
    expect(res.body.highlights['lic1']).toBe('Aquisição de <mark>equipamentos</mark>');
    // rank and headline should be stripped from data rows
    expect(res.body.data[0].rank).toBeUndefined();
    expect(res.body.data[0].headline).toBeUndefined();
  });
  ...
  it('falls back to ILIKE when tsvector query fails', async () => {
    ...
    expect(mockPrisma.licitacao.findMany).toHaveBeenCalled();
  });
  ...
  it('applies filters alongside tsvector search', async () => {
    ...
    await request(app).get('/api/licitacoes?q=equipamentos&esfera=FEDERAL&uf=SP');

    expect(mockPrisma.$queryRawUnsafe).toHaveBeenCalledTimes(2);
    // The SQL passed to $queryRawUnsafe should contain esfera and uf conditions
    const countSQL = mockPrisma.$queryRawUnsafe.mock.calls[0][0] as string;
    expect(countSQL).toContain('esfera');
    expect(countSQL).toContain('uf');
  });

  it('orders by relevance when q is provided and ordenarPor is relevancia', async () => {
    mockPrisma.$queryRawUnsafe
      .mockResolvedValueOnce([{ count: BigInt(1) }])
      .mockResolvedValueOnce([{ ...sampleLicitacao, rank: 0.9, headline: '<mark>equipamentos</mark>' }]);

    await request(app).get('/api/licitacoes?q=equipamentos&ordenarPor=relevancia');

    expect(mockPrisma.$queryRawUnsafe).toHaveBeenCalledTimes(2);
    // The data SQL (second call) should contain rank DESC ordering
    const dataSQL = mockPrisma.$queryRawUnsafe.mock.calls[1][0] as string;
    expect(dataSQL).toContain('rank');
