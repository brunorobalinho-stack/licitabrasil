...
export function FilterPanel() {
  const { filters, setFilters, resetFilters } = useFiltrosUrl();
  ...
  return (
    <aside className="w-full space-y-6 rounded-xl border bg-white p-5 dark:border-gray-700 dark:bg-gray-900">
        ...
        <div className="flex flex-wrap gap-2">
          {(Object.keys(ESFERA_LABELS) as Esfera[]).map((e) => (
            <ChipToggle
              ...
              onClick={() => setFilters({ esfera: filters.esfera === e ? undefined : e })}
            />
          ))}
        </div>
      </FilterSection>
      ...
      <FilterSection title="🗺️ Estado">
        <select
          value={filters.uf ?? ''}
          onChange={(e) => setFilters({ uf: e.target.value || undefined })}
      ...
      <FilterSection title="📋 Modalidade">
        <select
          ...
          onChange={(e) => setFilters({ modalidade: (e.target.value || undefined) as Modalidade | undefined })}
          className="w-full rounded-lg border px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-800"
        >
      ...
      <FilterSection title="📊 Status">
        <select
          ...
          onChange={(e) => setFilters({ status: (e.target.value || undefined) as StatusLicitacao | undefined })}
          className="w-full rounded-lg border px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-800"
        >
      ...
      {/* UASG */}
      <FilterSection title="🏛️ Código UASG">
        <input
          type="text"
          placeholder="Ex: 00001"
          value={filters.codigoUASG ?? ''}
          onChange={(e) => setFilters({ codigoUASG: e.target.value || undefined })}
          className="w-full rounded-lg border px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-800"
        />
      </FilterSection>
      ...
      <FilterSection title="📅 Data de Publicação">
        <div className="flex gap-2">
          <input
            ...
            onChange={(e) => setFilters({ dataPublicacaoInicio: e.target.value || undefined })}
            className="w-1/2 rounded-lg border px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-800"
          />
          <input
            type="date"
            value={filters.dataPublicacaoFim ?? ''}
            onChange={(e) => setFilters({ dataPublicacaoFim: e.target.value || undefined })}
            className="w-1/2 rounded-lg border px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-800"
          />
        </div>
      </FilterSection>
      ...
      <FilterSection title="🔃 Ordenar por">
        <select
          ...
          onChange={(e) => setFilters({ ordenarPor: e.target.value as 'dataPublicacao' | 'dataAbertura' | 'valorEstimado' | 'relevancia' })}
          className="w-full rounded-lg border px-3 py-2 text-sm dark:border-gray-600 dark:bg-gray-800"
        >
