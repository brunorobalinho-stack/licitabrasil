...
import { Building2, MapPin, Calendar, Clock, ExternalLink, Star } from 'lucide-react';
...
export const LicitacaoCard = memo(function LicitacaoCard({ licitacao, highlight, isAuthenticated, isFavorite }: Props) {
  const l = licitacao;
  const deadlineLabel = daysUntilLabel(l.dataAbertura);
  const toggle = useFavoritesStore((s) => s.toggle);
  const handleToggle = useCallback(() => toggle(l.id), [toggle, l.id]);
  ...
  return (
    <article className="group rounded-xl border bg-white p-5 shadow-sm transition-all hover:shadow-md hover:border-primary/30 dark:border-gray-700 dark:bg-gray-900">
      {/* Top row: Status + Modalidade + Edital + Fav */}
