...
import { Search, Bell, Star, LayoutDashboard, LogOut, Menu, X, Sun, Moon, Activity } from 'lucide-react';
    ...
    <header className="sticky top-0 z-50 border-b bg-white/80 backdrop-blur dark:bg-gray-950/80 dark:border-gray-800">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4">
        ...
        <nav className="hidden md:flex items-center gap-6">
          ...
          {isAdminOrAnalyst && (
            <Link to="/scrapers" className="flex items-center gap-1.5 text-sm font-medium hover:text-primary transition-colors">
              <Activity size={16} /> Fontes
            </Link>
          )}
        </nav>
      ...
      {mobileOpen && (
        <nav className="md:hidden border-t bg-white dark:bg-gray-950 p-4 space-y-3">
          ...
          {isAdminOrAnalyst && (
            <Link to="/scrapers" className="block py-2 text-sm font-medium" onClick={() => setMobileOpen(false)}>📡 Fontes</Link>
          )}
          {isAuthenticated && (
